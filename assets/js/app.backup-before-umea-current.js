...
(function () {
  ...
  function translatePage() {
    ...
    syncCurrentCampaignPromos();
  }

  function currentCampaignImageForLang(campaign) {
    if (!campaign) return '';
    const activeLang = lang();
    if (activeLang === 'sv' && campaign.imageUrlSv) return campaign.imageUrlSv;
    if (activeLang === 'ar' && campaign.imageUrlAr) return campaign.imageUrlAr;
    return campaign.imageUrl || campaign.imageUrlAr || campaign.imageUrlSv || '';
  }

  function syncCurrentCampaignPromos() {
    const campaign = data.currentCampaign || {};
    const imageUrl = currentCampaignImageForLang(campaign);
    const title = campaign.title || t('currentTitle');

    const homeImage = document.getElementById('homeCampaignPromoImage');
    const homeLink = document.getElementById('homeCampaignPromoLink');
